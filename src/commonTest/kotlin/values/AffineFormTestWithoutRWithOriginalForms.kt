...
class AffineFormTestWithoutRWithOriginalForms {

    private val precision = 0.000001

    /*
    private val af1 = AffineForm(this, 1.0, 2.0, 1)
    private val af2 = AffineForm(this, 1.0, 2.0, 2)
    ...
    /*init {
        ...
        terms[1] = 2.0
        terms[2] = 1.0
        lgr = AffineForm(this, Range.Reals,10.0, 0.0, terms)

        // Scalar form
        scl = AffineForm(this, 1.0)

        // Restricted range through manual min/max values
        terms = HashMap()
        terms[2] = 0.5
        rst = this.AffineForm(this, Range(1.1, 1.9), 1.5, 0.0, terms)
        ...
        termsGaussian["1"] = Pair(AffineForm(this, 1.0), Triple(0.0,0.5,0.0))
        termsGaussian["2"] = Pair(AffineForm(this, 1.0),Triple(1.0,0.5,0.0))
        gaussian1 = AffineForm(this, Range(1.0,2.0), 1.5, 0.0 ,terms2)
        ...
        termsGaussian2["1"] = Pair(AffineForm(this, 1.0), Triple(0.0,0.5,0.0))
        termsGaussian2["3"] = Pair(AffineForm(this, 0.5),Triple(0.5,0.5,0.0))
        gaussian2 = AffineForm(this, Range(0.0,2.0), 1.0, 0.0 , terms3)
        gaussian2.setXiGaussian(termsGaussian2)


        val termDNStmp = HashMap<String,Pair<AffineForm,Triple<Double,Double,Double>>>()
        termDNStmp["2"] = Pair(AffineForm(this, 1.5), Triple(1.0,0.5,0.0))
        val termAF = AffineForm(this, 0.5)
        termAF.setXiGaussian(termDNStmp)

        val termsGaussian3 = HashMap<String,Pair<AffineForm,Triple<Double,Double,Double>>>()
        termsGaussian3["1"] = Pair(AffineForm(this, 2.0), Triple(0.0,0.5,0.0))
        termsGaussian3["3"] = Pair(termAF,Triple(0.5,0.5,0.0))
        mixedTermDNS = AffineForm(this,  1.0)
    ...
    fun testAddFormsCustomRange() {
        DDBuilder {
            ...
            terms = HashMap()
            terms[2] = 0.5
            val rst = buildAF(this, 1.1 .. 1.9, 1.5, 0.0, terms)
    ...
    fun testUnproperRanges(){
        DDBuilder{
            ...
            val empty = buildAF(this, 0.0 .. 0.5, 1.5, 0.0, terms)
