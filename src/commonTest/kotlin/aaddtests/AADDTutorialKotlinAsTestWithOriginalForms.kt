...
class AADDTutorialKotlinAsTestWithOriginalForms {
    ...
    fun instantiation() {
        DDBuilder{
            ...
            val scalar   = real(1.0)
            val interval = real(2.0 .. 3.0, "x")  // min, max are doubles, index is int
            val real     = Reals
            val empty    = Empty
    ...
    fun bddInstantiation() {
        DDBuilder{
            ...
            val t = True
            val x = boolean("X")  // Boolean variable "X" that can take values true or false
            val d = (f and x) or t
            val e = t and x
            assertTrue(a === True)
    ...
    fun comparison3() {
        DDBuilder{
            this.config.originalFormsFlag = true
            val a = real(1.0 .. 3.0, "1")
            val b = real(2.0 .. 4.0, "2")
