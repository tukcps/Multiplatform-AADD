...
class AcosFunctionTests {
    @Test
    fun acosZero() = ddTest {
        assertSafeInclusion(PI / 2.0..PI / 2.0, acos(AffineForm.scalar(this, 0.0)), 1e-8)
    }

    @Test
    fun acosOne() = ddTest {
        assertSafeInclusion(0.0..0.0, acos(AffineForm.scalar(this, 1.0)), 1e-8)
    }

    @Test
    fun acosMinusOne() = ddTest {
        assertSafeInclusion(PI..PI, acos(AffineForm.scalar(this, -1.0)), 1e-8)
    }

    @Test
    fun acosHalf() = ddTest {
        assertSafeInclusion(PI / 3.0..PI / 3.0, acos(AffineForm.scalar(this, 0.5)), 1e-8)
    }

    @Test
    fun acosInterval() = ddTest {
        val y = acos(AffineForm.range(this, -0.5..0.5))
        assertSafeInclusion(PI / 3.0..2.0 * PI / 3.0, y, 1e-6)
    }

    @Test
    fun acosDomain() = ddTest {
        val y = acos(AffineForm.range(this, -1.0..1.0))
        assertSafeInclusion(0.0..PI, y, 1e-6)
    }
    ...
    fun acosPartialDomain() = ddTest {
        ...
        assertSafeInclusion(PI / 3.0..PI, y, 1e-6)
    }
