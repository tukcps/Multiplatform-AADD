...
internal class AADDTestsWithOriginalForms {

    private val PRECISION_EXP_MINUS_6 = 0.000001
    private val PRECISION_EXP_MINUS_2 = 0.01
    ...
    fun infeasibilityTest() {
        DDBuilder {
            ...
            val a = this.real(0.0..1.0, "n")
            val b = this.real(3.0..4.0, "n")
    ...
    @Test
    // Check if negation works for real: value negates is - value.
    fun negate() {
        DDBuilder {
            this.config.originalFormsFlag = true
            val a = real(10.0)
    ...
    fun expTest() {
        DDBuilder {
            ...
            val exp1 = a.exp()
            val exp2 = b.exp()
            assertEquals(exp(3.5), exp1.getRange().min, PRECISION_EXP_MINUS_6)
            assertEquals(exp(-1.0), exp2.getRange().min, PRECISION_EXP_MINUS_6)

            // Test Interval
            val c = leaf(affineForm1)
            val exp3 = c.exp() as AADD.Leaf
            assertEquals(5.06, exp3.central, PRECISION_EXP_MINUS_2)
            assertEquals(0.98, exp3.value.r, PRECISION_EXP_MINUS_2)
            assertEquals(2.72, exp3.min, PRECISION_EXP_MINUS_2)
            assertEquals(7.39, exp3.max, PRECISION_EXP_MINUS_2)
    ...
    fun testOfEulersNumber() {
        ...
        builder.apply {
            ...
            var d : AADD = a.exp()
            assertEquals(2.7182818284590446, d.getRange().min, PRECISION_EXP_MINUS_2)
            assertEquals(2.7182818284590446, d.getRange().max, PRECISION_EXP_MINUS_2)
            d = b.exp()
            assertEquals(0.3678794411714423, d.getRange().min, PRECISION_EXP_MINUS_2)
            assertEquals(0.3678794411714423, d.getRange().max, PRECISION_EXP_MINUS_2)
            d = c.exp()
            assertEquals(1.0, d.getRange().min, PRECISION_EXP_MINUS_2)
            assertEquals(1.0, d.getRange().max, PRECISION_EXP_MINUS_2)
        }
    }
    ...
    fun sqrtTest() {
        DDBuilder {
            ...
            assertEquals(1.0, sqrt1.min, PRECISION_EXP_MINUS_2)
            assertEquals(kotlin.math.sqrt(a.max), sqrt1.max, PRECISION_EXP_MINUS_2)
        }
    }
    ...
    fun logTest() {
        DDBuilder {
            ...
            assertEquals(0.38, log1.central, PRECISION_EXP_MINUS_2)
            assertEquals(0.02, log1.r, PRECISION_EXP_MINUS_2)
            assertEquals(0.0, log1.min, PRECISION_EXP_MINUS_2)
            assertEquals(ln(a.max), log1.max, PRECISION_EXP_MINUS_2)
            val b = leaf(largerValue)
            val log2 = b.log() as AADD.Leaf
            assertEquals(2.28, log2.central, PRECISION_EXP_MINUS_2)
            assertEquals(0.02, log2.r, PRECISION_EXP_MINUS_2)
            assertEquals(ln(b.min), log2.min, PRECISION_EXP_MINUS_2)
            assertEquals(ln(b.max), log2.max, PRECISION_EXP_MINUS_2)
            val c = leaf(restrictedRange)
            val log3 = c.log() as AADD.Leaf
            assertEquals(0.39, log3.central, PRECISION_EXP_MINUS_2)
            assertEquals(0.02, log3.value.r, PRECISION_EXP_MINUS_2)
            assertEquals(ln(c.min), log3.min, PRECISION_EXP_MINUS_2)
            assertEquals(ln(c.max), log3.max, PRECISION_EXP_MINUS_2)
    ...
    fun invTest() {
        DDBuilder {
            ...
            // Real -> Real
            val infNode = Reals
            val inv2 = infNode.inv() as AADD.Leaf
            assertTrue(inv2.value.isReals())

            // Regular
            val inv3Node = leaf(affineForm1)
            val inv3 = inv3Node.inv() as AADD.Leaf
            assertEquals(0.75, inv3.central, PRECISION_EXP_MINUS_6)
            assertEquals(0.5, inv3.min, PRECISION_EXP_MINUS_6)
            assertEquals(1.0, inv3.max, PRECISION_EXP_MINUS_6)
            assertEquals(0.125, inv3.r, PRECISION_EXP_MINUS_6)
            assertEquals(0.25, inv3.radius, PRECISION_EXP_MINUS_6)
        }
    }
    ...
    @Test fun iteTest2() {
        DDBuilder {
            this.config.originalFormsFlag = true
            var a: AADD = this.real(-1.0..1.0, "1")
            val b = real(0.0)
    ...
    fun testPowFxn() {
        ...
        builder.apply {
            ...
            val c = 3.0
            var d : AADD = pow(a, c)
            assertEquals(8.0, d.getRange().min, PRECISION_EXP_MINUS_2)
            assertEquals(8.0, d.getRange().max, PRECISION_EXP_MINUS_2)
            d = pow(a, b)
            assertEquals(4.0, d.getRange().min, PRECISION_EXP_MINUS_2)
            assertEquals(4.0, d.getRange().max, PRECISION_EXP_MINUS_2)
        }
    }
    /**
     *
